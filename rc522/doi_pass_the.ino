...
void loop() {
    ...
    byte trailerBuffer[] = {
        's', 'o', 'f', 'e', 'l', 'e',       // Keep default key A
        0, 0, 0,
        0,
        'e', 'l', 'e', 'f', 'o', 's'};      // Keep default key B
    ...
    mfrc522.MIFARE_SetAccessBits(&trailerBuffer[6], 0, 0, 0, 0);
    ...
    status = mfrc522.PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_B, trailerBlock, &key2, &(mfrc522.uid));
    if (status != MFRC522::STATUS_OK) {
        Serial.print(F("PCD_Authenticate() failed: "));
        Serial.println(mfrc522.GetStatusCodeName(status));
        return;
